...
def collate_fn(examples):
    original_embedding = torch.stack(
        [
            torch.tensor(example["original_embedding"]).reshape(12, -1)
...
def main():
    ...
    if adapter_args.load_adapter:
        ...
        for name, param in model.named_parameters():
            if "aligner" in name or "new_position" in name:
                param.requires_grad_ = True
    model.set_active_adapters(["tada_aave"])
    if data_args.use_eraser:
        for layer in model.iter_layers():
            modified_adapter = Eraser(
                adapter_name="tada_aave",
                input_size=model.config.hidden_size,
                down_sample=model.config.hidden_size,
                config=adapter_config,
            )
            layer[1].output.adapters["tada_aave"] = modified_adapter
        model = model.cuda()
