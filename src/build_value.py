...
def convert(args, D, directory):
    ...
    for in_fn in sorted(glob(os.path.join(args.GLUE, directory) + '/*.tsv')):
        ...
        with open(out_fn, 'w') as outfile:
            wrote_header = False
            header = []
            total_length = len(list(csv.reader(open(in_fn, 'r', encoding='utf-8-sig'), delimiter="\t", quotechar=None)))
                ...
                for row in tqdm(read_tsv, total=total_length):
                    if not wrote_header:
                        ...
                        outfile.write('\t'.join(out + \
                                               [str(modifications[which][key]) for which in sorted(modifications.keys()) for key in sorted(modifications[which].keys())]
                                               ) + '\n')
def main():
